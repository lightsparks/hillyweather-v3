                        ...
                        <v-window-item :value="1">
                            <v-card elevation="4">
                                ...
                                <v-card-text class="pt-0">
                                    ...
                                    <v-card-text
                                            class="d-flex justify-center font-weight-black pt-0 pb-0"
                                    >
                                        <h3>{{ "Min: " + today.minTemp + "°C" }}  ||  {{ "Max: " + today.maxTemp + "°C" }}</h3>
                                    </v-card-text>
                                </v-card-text>
                                ...
                                <v-list-item v-for="weekday in week" :key="weekday.dt">
                                    <v-card class="mx-auto mb-6" width="500">
                                        ...
                                        <v-container class="d-flex flex-row mb-0 pt-0 pb-0 align-content-center">
                                            <v-img
                                                    class="mx-auto align-self-center ml-3"
                                                    width="60px" alt="Weekday Icon"
                                                    :src="weekday.icon"
                                                    transition="scale-transition"
                                                    contain></v-img>
                                            <v-card-text
                                                    class="font-italic text-center ml-1 align-self-center text--secondary"
                                            >
                                                <h4>{{ weekday.weather[0].description }}</h4>
                                            </v-card-text>
                                            <v-card-text
                                                    class="mx-auto d-flex justify-end display-1 pl-0 pr-0 ml-0 font-weight-bold text--secondary"
                                            >{{ Math.round(weekday.temp.day) + "°C" }}
                                            </v-card-text>
                                        </v-container>
                                        <v-card-subtitle
                                                class="d-flex justify-end align-self-center pt-0"
                                                color="primary"
                                        >
                                            {{ "Min: " + Math.round(weekday.temp.min) + "°C" }}  ||  {{ "Max: " + Math.round(weekday.temp.max) + "°C" }}
                                        </v-card-subtitle>
                                    </v-card>
                                </v-list-item>
                            </v-list>
